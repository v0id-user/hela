...
| `HELA_GATEWAY_URL`        | dev gateway public URL                    | production gateway public URL              | single-region fallback; control resolves every hosted region to this URL unless `GATEWAYS` overrides per region |
| `GATEWAYS`                | optional                                  | optional                                   | per-region json overrides on top of `HELA_GATEWAY_URL`; only set when more than one gateway exists              |
...
railway variable set --service control --environment $ENV --skip-deploys \
  ...
  "HELA_GATEWAY_URL=https://$GW_HOST" \
